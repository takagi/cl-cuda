...
(defmethod glut:display ((w test-window))
  ...
  (let ((dev-id 0))
    (with-cuda-context (dev-id)
      (is-error (cl-cuda.api::alloc-memory-block 'int 1024 :interop t) simple-error)))
  ...
  (let ((dev-id 0))
    (with-cuda-context (dev-id :interop t)
      ...
      (let (blk)
        (ok (setf blk (cl-cuda.api::alloc-memory-block 'int 1024 :interop t)))
        (cl-cuda.api::free-memory-block blk))))
  ...
  (let ((dev-id 0))
    (with-cuda-context (dev-id :interop t)
      (with-memory-blocks ((blk 'int 1024 :interop t))
        ...
        (is-error (cl-cuda.api::memory-block-device-ptr blk) simple-error)
        (cl-cuda.api::with-memory-block-device-ptr (device-ptr blk)
          (ok device-ptr))
        (ok       (cl-cuda.api::memory-block-interop-p blk))
        (ok       (cl-cuda.api::memory-block-vertex-buffer-object blk))
        (ok       (cl-cuda.api::memory-block-graphic-resource-ptr blk))
