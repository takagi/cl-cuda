...
(defglobal origin float4)
(defglobal delta float)
(defglobal capacity int)
(defglobal size-x int)
  ...
  (let* (;; Grid and block dims.
    ...
    (multiple-value-bind (size-x size-y size-z size)
      ...
      (with-cuda (0)
        ...
        (setf (global-ref 'box-max) box-max)
        ;; Set neighbor map globals.
        (setf (global-ref 'origin) origin)
        (setf (global-ref 'delta) delta)
        (setf (global-ref 'capacity) capacity)
        (setf (global-ref 'size-x) size-x)
        (setf (global-ref 'size-y) size-y)
        (setf (global-ref 'size-z) size-z)
        ;; With memory blocks.
